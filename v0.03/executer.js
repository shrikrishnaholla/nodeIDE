...
function execute (client, code) {
    ...
    fs.writeFile("/tmp/test"+client.id+".c", code, function(error) {
        if (error) {
    ...
    function compiler () {
        cp.exec("cc -Wall /tmp/test"+client.id+".c -o /tmp/test"+client.id, function(e, stdout, stderr) {
            if (e !== null) {
    ...
    function exiter () {
        client.send('Done')
        fs.unlink('/tmp/test'+client.id+".c", function (err) {
          if (err) console.log("Error deleting file for client",client.id);
          console.log('successfully deleted /tmp/test'+client.id+".c");
        });
